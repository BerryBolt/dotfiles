...
Apply writes the token as `CLAUDE_CODE_OAUTH_TOKEN` into the `env` block of `~/.claude/settings.json` (mode 0600), so `claude -p` finds it whoever starts it: a terminal, a script, a service, or a Codex worker, whose commands never inherit token variables. Interactive `claude` is different: its first run walks through onboarding and asks for a login method even with the token set, exactly as it does with the variable exported. Anthropic documents the token as an environment variable and the `env` block as a way to set one; keeping the token there is this repo's choice. Claude Code passes `env` entries to the commands it runs, so those commands can read the token, as they can read `~/.config/op/env`.
