...
- The agent CLIs `codex` and `claude` come from Omarchy's launchers; the repo manages only their settings. They follow `latest` and are not pinned: each launcher runs `mise use -g <tool>` on every call, which writes `latest` into `~/.config/mise/config.toml`, and that file takes precedence over `conf.d/`. They upgrade when `omarchy-update` runs `mise up`.
